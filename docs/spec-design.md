...
- **App shell**: `src/App.tsx` renders a single-page timeline workspace with:
  - Fixed header (`src/components/Header.tsx`)
  - Scrollable timeline (`src/components/Timeline.tsx`)
  - Fixed compose footer (`src/components/ComposePanel.tsx`)
- **Timeline markers**:
...
- **Feedback**:
  - Forecast errors are shown via `Toast` (`src/components/Toast.tsx`).
