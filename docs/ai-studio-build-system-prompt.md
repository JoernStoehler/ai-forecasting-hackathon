# Google AI Studio Build Mode - Agent System Prompt

**Date Obtained:** 2026-01-21
**Source:** Extracted from AI Studio Build agent via prompt request
**Status:** May contain minor modifications/errors added by the agent during extraction

---

## Purpose

This document contains the system instructions that govern the AI Studio Build agent's behavior. It documents:
- The expected file structure (`index.html` + `index.tsx` with ES6 modules)
- API key handling requirements (`process.env.API_KEY`)
- The @google/genai SDK coding guidelines
- Output format requirements (XML-based file changes)

This helps understand:
1. Why complex TypeScript projects fail with ZIP upload (no build process mentioned)
2. What the actual runtime environment provides
3. How to structure projects for successful AI Studio Build deployment

**See also:** [AI Studio Build Knowledge Base](./ai-studio-build-knowledge.md) for complete knowledge with source citations
...
**Confirmed:**
- AI Studio Build accepts ZIP files with `index.html` + `index.tsx`
- Uses ES6 modules (`<script type="module">`)
- Environment provides `process.env.API_KEY` for Gemini API access
- Files must be at ZIP root (no nested `src/` directory)
- Agent returns changes in XML format with `<changes>` blocks

**Unclear/Undocumented:**
- How TypeScript files are processed (compiled? transpiled? interpreted?)
- Whether there's a build step on upload or if TS runs directly
- What Node.js/browser APIs are available in the runtime
- Bundle size limits or performance constraints
- How CSS processing works (our solution: pre-compile and include)
...
<changes>
  ...
  <change>
    ...
    <description>[description of change]</description>
    <content><![CDATA[Full content of file_2]]]]><![CDATA[></content>
  </change>
</changes>
```
