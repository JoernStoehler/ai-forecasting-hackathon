...
<!--
NOTE (2026-01-02): This file is written by Jörn, but agents may propose edits as part of repo maintenance.
Any agent edits are *not* considered accepted unless Jörn explicitly reviews and approves them.
The additions below are marked inline as "AGENT DRAFT" for clarity.
-->
...
- The LLM-based forecaster processes the player's turn, and updates the scenario state and history. Breaking down the forecaster step further:
  ...
  <!-- AGENT DRAFT (unvetted by Jörn): added 2026-01-02 based on owner guidance -->
  - [PROPOSED] Intended pacing: each game master turn should advance the timeline by roughly ~6 months beyond the latest known in-world date (not necessarily by adding an event for every month; 1–5 key “news” items are fine).
  - [PROPOSED] The forecaster output should be *softly chronological* in in-world time (so the player sees the story progress forward as they scroll). Corrections are allowed: the forecaster may patch earlier items when needed, but should avoid retroactive edits unless they clearly improve coherence or fix mistakes.
...
- We record the engine state using an event sourcing pattern, i.e. just dump all commands into a log, and reduce to get the state. We don't mind spamming a lot of UI effects into the log files, since the LLM prompt is filtered and optimized for LLM consumption anyway later.
  ...
  <!-- AGENT DRAFT (unvetted by Jörn): added 2026-01-02 based on owner guidance -->
  - [PROPOSED] Important token-budget rule: we only worry about token count / verbosity in (a) the prompt projection we send to the game master and (b) the game master’s output Commands. We do **not** try to keep the raw event log small: it exists for reproducibility, debugging, and future aggregation, and may include high-fidelity player UI intent/telemetry.
  - [PROPOSED] Concretely: the event log can be “chatty”, but the prompt projection must be “calm”. The projection is allowed to compress, aggregate, and omit low-value detail (while staying faithful) so the forecaster can focus on forecasting rather than on reading raw UI noise.
  - [PROPOSED] Optional: we can either treat state as a pure function that reduces the sequence of commands, and we record all commands, or we distinguish between ephemeral state (e.g. UI in React sometimes has a lot of that) and reproducible state (semantic content of the scenario and all we count towards it, i.e. maybe the list of opened reports so far is semantic information about the policy wargame). Reproducible state is a reduced function of events, where events are emitted by commands. Ephemeral state is a reduced function of events as well, again emitted by commands, but potentially a different set of events. I.e., commands emit two kinds of events, that get reduced into two states, where the reproducible state logs the events, while the ephemeral state doesn't log them but just applies them and forgets them. This way we conceputally don't bloat the event log with every pixel movement of the mouse cursor.
  - [PROPOSED] Potentially, the ephemereal state is entirely under the hood, i.e. we just let the browser handle it, and everything we actually code interacts with the reproducible state and event log only. In that case, we may even want commands to emit a single event only.
...
<!-- AGENT DRAFT (unvetted by Jörn): added 2026-01-02 based on owner guidance -->
- [PROPOSED] Materials in the prompt are not a player-facing “picker” feature. We do **not** plan to add a GUI where players manually decide which materials the game master can see. Instead, materials inclusion should be a deterministic/algorithmic choice made by the game (MVP default: include everything; later: include only relevant subsets to reduce distraction and keep the forecaster focused).
- [PROPOSED] We could do branching timelines, but honestly, feels boring. Better to have players explore a highly uncertain scenario landscape with initial randomness + calibrated unpredictability (i.e. the forecasting AI will have dice to better balance how surprising its predictions are bc LLMs sometimes have positivity bias, or lean towards predictable median cases, or fall for typical reasoning biases like conjunction fallacy, UNLESS we prompt them to roll dice and think quantitatively about what kind of outcome to produce).
- [PROPOSED] Since we can only inject randomness into the prompts, I suggest that we basically use a Command for turn change that setups the PRNG, e.g. the prompt always contains a line "After the next 6 months, total AI capability growth has fallen into the 76th percentile (in standard deviations: +0.7) compared to what expectations a calibrated expert forecaster has at the start of the 6 month interval. Formulate your forecast, i.e. interpret the percentile, then flesh it out into a full 6 month timeline that you communicate as Commands[] output."
...
<!-- AGENT DRAFT (unvetted by Jörn): added 2026-01-02 based on owner guidance -->
- [PROPOSED] Single source of truth for scenario seed events and materials across frontends: the webapp and CLI are interchangeable adapters over the same engine + content bundle, not separate games.
...
<!-- AGENT DRAFT (unvetted by Jörn): added 2026-01-02 based on owner guidance -->
- [PROPOSED] [ ] AI Studio Build test deployment path + smoke checklist
- [PROPOSED] [ ] Turn segmentation markers + attention telemetry (so the GM can see “what turn are we in” and “what did the player look at” without drowning in raw UI)
- [PROPOSED] [ ] Minimal regression tests for the core loop (smoke/E2E)
- [PROPOSED] [ ] Clear stance + UX for scenario branching (if ever)
