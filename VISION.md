...
- The LLM-based forecaster processes the player's turn, and updates the scenario state and history. Breaking down the forecaster step further:
  ...
  - Intended pacing: each game master turn should advance the timeline by roughly ~6 months beyond the latest known in-world date (not necessarily by adding an event for every month; 1–5 key “news” items are fine).
  - The forecaster output should be *softly chronological* in in-world time (so the player sees the story progress forward as they scroll). Corrections are allowed: the forecaster may patch earlier items when needed, but should avoid retroactive edits unless they clearly improve coherence or fix mistakes.
...
- We record the engine state using an event sourcing pattern, i.e. just dump all commands into a log, and reduce to get the state. We don't mind spamming a lot of UI effects into the log files, since the LLM prompt is filtered and optimized for LLM consumption anyway later.
  ...
  <!-- AGENT DRAFT (unvetted by Jörn): added 2026-01-02 based on owner guidance -->
  - Important token-budget rule: we only worry about token count / verbosity in (a) the prompt projection we send to the game master and (b) the game master’s output Commands. We do **not** try to keep the raw event log small: it exists for reproducibility, debugging, and future aggregation, and may include high-fidelity player UI intent/telemetry.
  - Concretely: the event log can be “chatty”, but the prompt projection must be “calm”. The projection is allowed to compress, aggregate, and omit low-value detail (while staying faithful) so the forecaster can focus on forecasting rather than on reading raw UI noise.
...
<!-- AGENT DRAFT (unvetted by Jörn): added 2026-01-02 based on owner guidance -->
- Materials in the prompt are not a player-facing “picker” feature. We do **not** plan to add a GUI where players manually decide which materials the game master can see. Instead, materials inclusion should be a deterministic/algorithmic choice made by the game (MVP default: include everything; later: include only relevant subsets to reduce distraction and keep the forecaster focused).
...
<!-- AGENT DRAFT (unvetted by Jörn): added 2026-01-02 based on owner guidance -->
- Single source of truth for scenario seed events and materials across frontends: the webapp and CLI are interchangeable adapters over the same engine + content bundle, not separate games.
...
<!-- AGENT DRAFT (unvetted by Jörn): added 2026-01-02 based on owner guidance -->
- [ ] AI Studio Build test deployment path + smoke checklist
- [ ] Turn segmentation markers + attention telemetry (so the GM can see “what turn are we in” and “what did the player look at” without drowning in raw UI)
