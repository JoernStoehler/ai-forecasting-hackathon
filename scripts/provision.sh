...
sync_env_local_from_env() {
  ...
  if [ -n "$gemini_val" ] || [ -n "$tavily_val" ]; then
    ...
    echo "[provision] wrote .env.local from available API secrets"
  else
    if [ -f "$env_path" ]; then
      echo "[provision] API secrets not set; kept existing .env.local"
    else
      printf "GEMINI_API_KEY=PLACEHOLDER\n" > "$env_path"
      chmod 600 "$env_path" || true
      echo "[provision] API secrets not set; created placeholder .env.local"
    fi
  fi
}
...
write_persisted_env_exports() {
  ...
  if [ -z "$gemini_val" ] && [ -z "$tavily_val" ]; then
    if [ -f "$secrets_path" ]; then
      echo "[provision] API secrets not set; kept existing $secrets_path"
    else
      echo "[provision] API secrets not set; no persisted exports created"
    fi
    return 0
  fi
  ...
  mkdir -p "$secrets_dir"
  {
    ...
    echo "# Autogenerated by scripts/provision.sh"
    if [ -n "$gemini_val" ]; then
  ...
  echo "[provision] wrote persisted API exports to $secrets_path"
}
...
ensure_shell_sources_persisted_env() {
  ...
  if rg -Fq "$marker_start" "$rc_file" >/dev/null 2>&1; then
    echo "[provision] $(basename "$rc_file") already sources persisted env"
    return 0
  fi
  ...
  echo "[provision] configured $(basename "$rc_file") to source persisted env"
}
