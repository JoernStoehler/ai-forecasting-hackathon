...
export const Timeline: React.FC<TimelineProps> = ({ events, searchQuery, scenarioHeadDate }) => {
  ...
  return (
    <div className="relative pt-4">
            ...
            <div key={month} className="relative">
              ...
              {monthEvents.flatMap((event, index) => {
                ...
                if (
                  scenarioHeadDate &&
                  !cutoffInserted &&
                  event.date > scenarioHeadDate
                ) {
                  items.push(
                    <ForecastMarker key={`forecast-marker-${scenarioHeadDate}`} date={scenarioHeadDate} />
                  );
                  cutoffInserted = true;
                }
                items.push(
      ...
      {scenarioHeadDate && !cutoffInserted && (
        <ForecastMarker date={scenarioHeadDate} />
      )}
    </div>
  );
};
